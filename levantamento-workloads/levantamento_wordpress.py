#!/usr/bin/env python3
"""
levantamento_wordpress.py — Levantamento de infraestrutura WordPress/Sites
Foco: EC2, ASG, ECS, RDS, ALB, CloudFront, WAF, EFS + findings de segurança.
Use para workloads com padrão CloudFront → ALB → EC2 → RDS.

Rodar no AWS CloudShell da conta-alvo (sem assume role).
Permissões necessárias: ReadOnlyAccess ou ViewOnlyAccess + SecurityAudit
...
def main():
    ...
    titulo(f"LEVANTAMENTO DE INFRAESTRUTURA WORDPRESS/SITES")
